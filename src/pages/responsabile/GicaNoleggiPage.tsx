...
import { dreamClientiApi, dreamVeicoliApi, gicaNoleggiApi } from '../../api/client';
import type {
  DreamCliente,
  DreamVeicolo,
  GicaNoleggio,
...
export function GicaNoleggiPage() {
  ...
  const [noleggi, setNoleggi] = useState<GicaNoleggio[]>([]);
  ...
  // Il formato in corso, non un booleano: due pulsanti e un solo spinner acceso
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await gicaNoleggiApi.getAll(dal, al);
        if (cancelled) return;
  ...
  const totale = useMemo(
    () => noleggi.reduce((acc, n) => acc + n.importo, 0),
    [noleggi]
  );
  ...
  const handleEdit = (noleggio: GicaNoleggio) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: GicaNoleggioInput = {
    ...
    setIsSaving(true);
    try {
      if (editId !== null) await gicaNoleggiApi.update(editId, payload);
      else await gicaNoleggiApi.create(payload);
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await gicaNoleggiApi.delete(deleteId);
  ...
  const handleExport = async (formato: 'pdf' | 'excel') => {
    ...
    try {
      if (formato === 'pdf') await gicaNoleggiApi.exportPdf(dal, al);
      else await gicaNoleggiApi.exportExcel(dal, al);
    } catch {
  ...
  return (
    <ResponsabileLayout>
      ...
      {/* Il form sta in una card e non in un Modal: cinque campi in max-w-md
            ...
            <table className="w-full text-sm">
              ...
              <tfoot>
                <tr className="border-t-2 font-medium bg-gray-50">
                  ...
                  <td className="py-3 px-2 text-right whitespace-nowrap">{formatImporto(totale)}</td>
                  <td />
                </tr>
              </tfoot>
            </table>
